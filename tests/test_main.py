...
def test_run_tariffs_prints_unicode_pretty_json(
    json_output: bool,
    capsys: pytest.CaptureFixture[str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    response = {"result": {"tariffs": [{"name": "Инвестор"}]}}
    use_case = FakeGetTariffs(response)
    composition_calls: list[tuple[str, str]] = []
    monkeypatch.setattr(main_module, "load_settings", lambda *args, **kwargs: settings)

    def fake_create(public: str, private: str) -> FakeGetTariffs:
        composition_calls.append((public, private))
        return use_case
...
@pytest.mark.parametrize(
    ...
    [
        (["tariffs"], {}),
        (["tariffs", "--json"], {"json_output": True}),
    ],
)
def test_main_routes_tariffs(
    arguments: list[str],
    expected_options: dict[str, object],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert main_module.main(arguments) == 17
    assert calls == [("tariffs", expected_options)]
...
def test_main_rejects_invalid_tariffs_before_orchestration(
    arguments: list[str],
    capsys: pytest.CaptureFixture[str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    orchestration_calls: list[str] = []
    use_case = FakeGetTariffs()
    monkeypatch.setattr(
        main_module,
        "run",
        lambda *args, **kwargs: orchestration_calls.append("run"),
